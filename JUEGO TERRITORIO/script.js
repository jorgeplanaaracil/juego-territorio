...
for (let i = 0; i < size; i++) {
  ...
  for (let j = 0; j < size; j++) {
    ...
    // -------------------------
    // 🔹 CASO 2: CONQUISTA (celda enemiga)
    // -------------------------
...
function actualizarFondoTurno() {
  ...
  if (turno === 1) {
    document.body.classList.add("turno-1");
  } else {
    document.body.classList.add("turno-2");
  }
}

// -------------------- REGLA DE CAPTURAR CELDAS --------------------
...
function capturarCeldas(jugador) {
  ...
  while (cambios) {
    ...
    // 🔹 Todas las reglas pasan por el sistema general
    ...
    // 👉 añade aquí más combinaciones si quieres
  }
...
function puedeConquistar(x, y, jugador) {
  ...
  let bloqueVertical = false;
  let bloqueHorizontal = false;
  let tieneContacto = false;

  // -------------------------
  // 🔹 contacto con territorio propio
  // -------------------------
  if (
  ...
  if (
    ...
    bloqueHorizontal = true;
  }

  // ❌ reglas finales
  if (!tieneContacto) return false;
  if (bloqueVertical || bloqueHorizontal) return false;
...
function comprobarFinJuego() {
  ...
  for (let i = 0; i < size; i++) {
    ...
    // 🔹 UI de puntuación en tiempo real
  ...
  if (total === size * size) {
    ...
    if (p1 > p2) {
      ganadorTexto.innerText = "Gana Jugador 1 🔵";
    } else if (p2 > p1) {
      ganadorTexto.innerText = "Gana Jugador 2 🔴";
    } else {
      ganadorTexto.innerText = "Empate ⚖️";
    }
  }
}
